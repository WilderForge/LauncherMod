...
public class Installer<Game extends IDownloadable & IGame> {
	...
	private final boolean isCoremodded;
	private final boolean visible;
	
	public Installer(Game game, Path dir, boolean installCoremodEnvironment) {
		this(game, dir, JVMInstance.thisVM(), installCoremodEnvironment, true);
	}
	
	public Installer(Game game, Path dir, JVMBinary jvm, boolean installCoremodEnvironment, boolean visible) {
		this.LOGGER = LogManager.getLogger(getClass());
		this.marker = MarkerManager.getMarker(game.toString());
		this.game = game;
		this.dir = dir;
		this.jvm = jvm;
		this.isCoremodded = installCoremodEnvironment;
		this.visible = visible;
	}
	
	@Deprecated
	@SuppressWarnings("unchecked")
	public Installer(Install install) {
		this.game = (Game) install.installableGame();
		this.marker = MarkerManager.getMarker(game.toString());
		this.LOGGER = LogManager.getLogger(getClass());
		
		this.dir = install.installPath();
		this.jvm = install.java();
		this.isCoremodded = install.isCoremodded();
		this.visible = install.visible();
	}
	
	public void install() throws InstallException {
		try {
			if(game instanceof WildermythManifest) {
				installThrixlvault(new Install<>((WildermythManifest)game, dir, JVMInstance.thisVM(), isCoremodded, visible));
			}
	...
	public void installThrixlvault(Install<WildermythManifest> install) throws IOException, InterruptedException, IntegrityException, ExecutionException {
		...
		try {
			...
			if(!GameStateChecker.gameInstanceOnClasspath()) {
				throw new IOException(new IllegalStateException("No steam context available. Must install from disk first."));
			}
			MassDownloadWeaver downloader = new MassDownloadWeaver(SteamManager.getUserPersonaName(), Set.of(manifest));
